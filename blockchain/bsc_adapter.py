...
This module provides a native, no-fallback adapter for Binance Smart Chain (BSC) blockchain.
It implements all required functionality for BSC interaction, including:
- Connection to BSC networks (mainnet, testnet)
- Balance checking
- Transaction creation, signing, and broadcasting
- Fee estimation
- Network status monitoring

Uses Web3.py for BSC integration with no fallback mechanisms to ensure reliable operation.
"""
...
class BSCAdapter(BaseBlockchainAdapter):
    """
    Native Binance Smart Chain (BSC) blockchain adapter.
    
    Provides direct interaction with BSC networks with no fallbacks.
    ...
    def __init__(self, network: str = 'mainnet', endpoint: Optional[str] = None):
        ...
        if self.network not in self.NETWORKS:
            raise ValueError(f"Unsupported BSC network: {network}")
        ...
        logger.info(f"Initialized BSC adapter for {self.network_name}")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not self.web3.is_connected():
                ...
                if not self.endpoint or self.endpoint == self.network_config['rpc_endpoints'][0]:
                    for backup_endpoint in self.network_config['rpc_endpoints'][1:]:
                        logger.info(f"Trying backup BSC endpoint: {backup_endpoint}")
                ...
                if not self.web3.is_connected():
                    raise BlockchainConnectionError(f"Could not connect to any BSC {self.network_name} endpoints")
            ...
            logger.info(f"Connected to BSC {self.network_name}, chain ID: {chain_id}")
            return True
            ...
            self.is_connected = False
            error_msg = f"Failed to connect to BSC {self.network_name}: {str(e)}"
    ...
    def get_balance(self, address: str) -> Decimal:
        ...
        Args:
            address: BSC address to check
            
        Returns:
            Decimal: Balance in BNB
        ...
        if not self.is_connected or not self.web3:
            raise BlockchainConnectionError("Not connected to BSC network")
            
        try:
            # Validate address
            if not self.validate_address(address):
                raise ValidationError(f"Invalid BSC address: {address}")
            ...
            # Convert to BNB and return as Decimal
    ...
    def create_transaction(self, **kwargs) -> TxParams:
        ...
        Returns:
            TxParams: BSC transaction parameters
        ...
        try:
            ...
            value = kwargs.get('value', 0)  # In BNB
            ...
            # Handle gas pricing (BSC typically uses legacy gas pricing)
            ...
            error_msg = f"Failed to create BSC transaction: {str(e)}"
            logger.error(error_msg)
            raise TransactionError(error_msg) from e
            
    @strict_blockchain_operation
    def sign_transaction(self, transaction: TxParams, private_key: str) -> Dict[str, Any]:
        """Sign a BSC transaction with private key.
        
        Args:
            transaction: The transaction parameters to sign
            private_key: BSC private key (hex string with or without 0x prefix)
        ...
        try:
            ...
            error_msg = f"Failed to sign BSC transaction: {str(e)}"
            logger.error(error_msg)
            raise TransactionError(error_msg) from e
    ...
    def broadcast_transaction(self, signed_transaction: Dict[str, Any]) -> str:
        ...
        try:
            ...
            error_msg = f"Failed to broadcast BSC transaction: {str(e)}"
            logger.error(error_msg)
    ...
    def get_transaction_status(self, tx_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Failed to get BSC transaction status for {tx_hash}: {str(e)}"
            logger.error(error_msg)
            raise BlockchainConnectionError(error_msg) from e
    ...
    def get_network_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # BSC may support EIP-1559, check for baseFeePerGas
            ...
            self.is_connected = False
            error_msg = f"Failed to get BSC network status: {str(e)}"
            logger.error(error_msg)
            raise BlockchainConnectionError(error_msg) from e
            
    @strict_blockchain_operation
    def estimate_fee(self, transaction: TxParams) -> Dict[str, Any]:
        """Estimate fee for BSC transaction.
        ...
        try:
            ...
            # Calculate fee in Wei and BNB
            fee_wei = gas_price * gas_limit
            fee_bnb = self.web3.from_wei(fee_wei, 'ether')
            
            # Get fee estimates for different priorities
            # BSC often has congestion, so we'll provide different priority levels
            try:
                # Generate fee estimates based on current gas price
                slow_gas_price = int(gas_price * 0.9)  # 90% of current
                normal_gas_price = gas_price  # Current gas price
                fast_gas_price = int(gas_price * 1.5)  # 50% premium
                rapid_gas_price = int(gas_price * 2)  # 100% premium
                
                # Calculate fee estimations
                fee_estimates = {
                    'slow': {
                        'gas_price_wei': slow_gas_price,
                        'gas_price_gwei': self.web3.from_wei(slow_gas_price, 'gwei'),
                        'fee_wei': slow_gas_price * gas_limit,
                        'fee_bnb': self.web3.from_wei(slow_gas_price * gas_limit, 'ether')
                    },
                    'normal': {
                        'gas_price_wei': normal_gas_price,
                        'gas_price_gwei': self.web3.from_wei(normal_gas_price, 'gwei'),
                        'fee_wei': normal_gas_price * gas_limit,
                        'fee_bnb': self.web3.from_wei(normal_gas_price * gas_limit, 'ether')
                    },
                    'fast': {
                        'gas_price_wei': fast_gas_price,
                        'gas_price_gwei': self.web3.from_wei(fast_gas_price, 'gwei'),
                        'fee_wei': fast_gas_price * gas_limit,
                        'fee_bnb': self.web3.from_wei(fast_gas_price * gas_limit, 'ether')
                    },
                    'rapid': {
                        'gas_price_wei': rapid_gas_price,
                        'gas_price_gwei': self.web3.from_wei(rapid_gas_price, 'gwei'),
                        'fee_wei': rapid_gas_price * gas_limit,
                        'fee_bnb': self.web3.from_wei(rapid_gas_price * gas_limit, 'ether')
                    }
                }
            except Exception as e:
            ...
            error_msg = f"Failed to estimate fee for BSC transaction: {str(e)}"
            logger.error(error_msg)
            raise TransactionError(error_msg) from e
            
    def disconnect(self) -> bool:
        """Disconnect from BSC network.
        ...
        self.is_connected = False
        self.web3 = None
        logger.info(f"Disconnected from BSC {self.network_name}")
        return True
